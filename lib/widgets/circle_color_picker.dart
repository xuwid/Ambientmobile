      ...
      child: Stack(
        children: <Widget>[
          _HuePicker(
            ...
            thumbColor:
                widget.isWhiteLight ? widget.controller!.selectedColor : null,
            onEnded: _onEnded,
            onChanged: (hue) {
              _hueController.value = hue;
              if (widget.isWhiteLight) {
                if (hue >= 0 && hue < 120) {
                  // Gradient from Color(0xFFFFC58F) to Color(0xFFFFFFFF)
                  double t = (hue - 0) / 120;
                  widget.selectedColor = Color.lerp(
                      const Color(0xFFFFC58F), const Color(0xFFFFFFFF), t)!;
                } else if (hue >= 120 && hue <= 240) {
                  // Gradient from Color(0xFFFFFFFF) to Color(0xFFC9E2FF)
                  double t = (hue - 120) / 120;
                  widget.selectedColor = Color.lerp(
                      const Color(0xFFFFFFFF), const Color(0xFFC9E2FF), t)!;
                } else {
                  // Gradient from Color(0xFFC9E2FF) back to Color(0xFFFFC58F)
                  double t = (hue - 240) / 120;
                  widget.selectedColor = Color.lerp(
                      const Color(0xFFC9E2FF), const Color(0xFFFFC58F), t)!;
                }
                widget.controller?.selectedColor = widget.selectedColor;
          ...
          children: <Widget>[
            SizedBox.expand(
              child: Padding(
                ...
                child: CustomPaint(
                  painter:
                      _CirclePickerPainter(widget.strokeWidth, widget.colors),
                ),
              ),
            ),
            Positioned(
